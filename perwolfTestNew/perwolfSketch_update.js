...
function updateFontPath() {    
    if (font) {
        ...
        svgCanvas.resizeCanvas(newWidth, newHeight); // Resize the SVG canvas
...
function saveIMG() {
  ...
  svgCanvas.noFill();
  svgCanvas.stroke(0);
  svgCanvas.strokeWeight(outline);

  for (let cmd of fontPath.commands) {
    if (cmd.type === 'M') {
      svgCanvas.beginShape();
      let pt = distort(cmd.x, cmd.y);
      svgCanvas.vertex(pt.x, pt.y);
    } else if (cmd.type === 'L') {
      let pt = distort(cmd.x, cmd.y);
      svgCanvas.vertex(pt.x, pt.y);
    } else if (cmd.type === 'C') {
      ...
      let pt2 = distort(cmd.x2, cmd.y2);
      let pt3 = distort(cmd.x, cmd.y);
      svgCanvas.bezierVertex(pt1.x, pt1.y, pt2.x, pt2.y, pt3.x, pt3.y);
    } else if (cmd.type === 'Q') {
      let pt1 = distort(cmd.x1, cmd.y1);
      let pt2 = distort(cmd.x, cmd.y);
      svgCanvas.quadraticVertex(pt1.x, pt1.y, pt2.x, pt2.y);
    } else if (cmd.type === 'Z') {
      svgCanvas.endShape(CLOSE);
    }
  }

  svgCanvas.save('perwolf.svg');
}
